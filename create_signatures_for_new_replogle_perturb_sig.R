...
config <- list(
  ...
  data_input_dir = file.path("/restricted/projectnb/brcameta/projects/sig_recon/data/sigs/perturb-seq"),
  k562_data_file = "k562_pb_deseq_tables.rds",
  rpe1_data_file = "rpe1_pb_deseq_tables.rds",
...
all_gene_symbols_k562 <- perturb_data$k562$gene
all_gene_symbols_rpe1 <- perturb_data$rpe1$gene
...
create_perturb_omic_signature <- function(
  ...
  difexp_data_clean <- current_results_df %>%
    dplyr::mutate(
      ...
      temp_avg_log2FC = as.numeric(log2FoldChange),
      temp_p_val_adj = as.numeric(padj),
      temp_p_val = as.numeric(pvalue),
  ...
  metadata_object <- OmicSignature::createMetadata(
    signature_name = paste0(perturbation_gene_symbol, "-KD_", cell_line),
    organism = "Homo sapiens",
...
for (gene_sym in unique(k562_ps_sig_all$pb)) {
  omic_sig <- create_perturb_omic_signature(
    perturbation_df = k562_ps_sig_all %>% dplyr::filter(pb == gene_sym),
...
for (gene_sym in unique(rpe1_ps_sig_all$pb)) {
  omic_sig <- create_perturb_omic_signature(
    perturbation_df = rpe1_ps_sig_all %>% dplyr::filter(pb == gene_sym),
    perturbation_gene_symbol = gene_sym,
    cell_line = "RPE1",
    gene_symbol_to_ensembl_map = gene_symbol_to_ensembl,
    adj_p_cutoff = adj_p_cutoff,
