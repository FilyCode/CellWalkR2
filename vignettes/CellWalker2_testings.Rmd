...
imputeATACtoRNAbySpatial <- function(rna_spatial_coords, atac_spatial_coords, atac_gene_activity_matrix, k_impute_spatial = 5) {
  ...
  imputed_atac_matrix <- Matrix::Matrix(0, nrow = nrow(rna_spatial_coords), ncol = ncol(atac_gene_activity_matrix), sparse = TRUE)
  rownames(imputed_atac_matrix) <- rownames(rna_spatial_coords)
  colnames(imputed_atac_matrix) <- colnames(atac_gene_activity_matrix)

  for (i in 1:nrow(rna_spatial_coords)) {
    neighbor_indices <- nn_result$nn.idx[i, ]
    if (k_impute_spatial == 1) { 
      imputed_profile <- atac_gene_activity_matrix[neighbor_indices, ]
    } else { 
      imputed_profile <- Matrix::colMeans(atac_gene_activity_matrix[neighbor_indices, , drop = FALSE])
    }
...
#' ATAC-seq gene activity scores. It uses `CellWalkR::computeCellSim` to generate a sparse
#' graph, representing cell-cell similarity for use in CellWalker.
#'
#' @param atac_gene_activity_matrix Sparse matrix of ATAC-seq gene activity scores (cells x genes).
#' @param method Character, method for ATAC feature similarity (e.g., "sparseJaccard", "spearman").
#' @return A sparseMatrix of cell-to-cell similarity, with values normalized
#'         between 0 and 1, where 1 indicates perfect similarity. Rownames and
#'         colnames are cell barcodes. Returns an empty (0x0) or 1x1 identity matrix
#'         if 0 or 1 cells remain after processing.
#' @export
createATACCellGraph <- function(atac_gene_activity_matrix, method = "sparseJaccard") {
  message("Computing ATAC-based cell-cell graph from gene activity matrix...")
  ...
  # Compute similarity using CellWalkR's computeCellSim
  atac_cell_graph <- CellWalkR::computeCellSim(atac_gene_activity_matrix[, non_zero_genes, drop = FALSE], method = method)
  
  # Ensure graph is aligned to original ATAC cell IDs and is symmetric with diagonal 1
  # (computeCellSim should generally preserve names, but a safeguard is good)
  if (!identical(rownames(atac_cell_graph), cell_ids) || !identical(colnames(atac_cell_graph), cell_ids)) {
      warning("Cell names not preserved by computeCellSim for ATAC graph. Attempting re-alignment. This might be inefficient for large datasets if names are significantly mismatched.")
      realigned_graph <- Matrix::Matrix(0, num_cells, num_cells, sparse = TRUE, dimnames = list(cell_ids, cell_ids))
      common_cells <- intersect(cell_ids, rownames(atac_cell_graph))
      realigned_graph[common_cells, common_cells] <- atac_cell_graph[common_cells, common_cells]
      atac_cell_graph <- realigned_graph
  }
...
#' This function does NOT combine the graphs but returns them individually for separate analysis
#' or validation.
#'
#' @param seurat_obj_rna_preprocessed Preprocessed Seurat object for spatial RNA-seq.
#' @param atac_counts_matrix_filtered Sparse matrix of ATAC-seq peak counts (cells x peaks).
...
generateSeparateModalGraphs <- function(
  ...
  # Ensure empty graph handling if creation failed
  rna_cell_ids <- colnames(seurat_obj_rna_preprocessed)
  ...
  # 3. Create ATAC Feature Graph
  atac_graph <- createATACCellGraph(atac_gene_activity_matrix, method = atac_feature_sim_method)
  # Ensure empty graph handling if creation failed
...
graph_params <- list(
  rna_feature_dims = 1:30,
  rna_feature_k_neighbors = 20,
  atac_feature_sim_method = "sparseJaccard" # Or "spearman" if more appropriate for data, need to check
)
...
separate_graphs <- generateSeparateModalGraphs(
  ...
  atac_feature_sim_method = graph_params$atac_feature_sim_method
)
...
if (is.null(seurat_obj_rna_preprocessed$seurat_clusters)) {
  seurat_obj_rna_preprocessed <- FindNeighbors(seurat_obj_rna_preprocessed, dims = rn_atac_graph_params$rna_feature_dims)
...
if (nrow(mock_markers_rna) == 0) {
  ...
  # Create dummy markers if none are found, to avoid crashing computeTypeEdges
  dummy_genes <- rownames(seurat_obj_rna_preprocessed)[1:10] # Take first 10 genes
...
if (nrow(rna_label_expr_norm_filtered) == 0) {
  stop("No common genes between RNA expression data and mock markers. Cannot compute label edges.")
}

rna_label_edges <- computeTypeEdges(rna_label_expr_norm_filtered, mock_markers_rna_df)
...
message("Running walkCells with the combined RNA+ImputedATAC graph and RNA-derived label edges...")
cell_walk_results_integrated <- walkCells(
  cellEdges = combined_rna_atac_graph,
  labelEdgesList = label_edges_list_walk,
  labelEdgeWeights = label_edge_weights_walk,
  sampleDepth = min(2000, nrow(combined_rna_atac_graph)) 
)

message("CellWalker results obtained with integrated RNA+ImputedATAC multi-omics:")
print(head(cell_walk_results_integrated$cellLabels))
```


```{r Spatial Validation of the Graph}
p_spatial_coherence <- plotSpatialGraphCoherence(
  combined_rna_atac_graph,
  rna_spatial_coords_filtered,
  n_query_cells = 3, # Plot 3 random query cells
  k_neighbors_to_show = 10 # Show 10 graph and 10 spatial neighbors
)
print(p_spatial_coherence)
```
