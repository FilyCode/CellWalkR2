...
#'
#' @param seurat_obj Seurat object containing RNA counts (assay = "RNA", slot = "counts").
...
simulatePseudoBulkRNA <- function(seurat_obj,
  ...
  mat <- as.matrix(Seurat::GetAssayData(seurat_obj, assay = "RNA", slot = "counts"))
...
plotSpatialGraphCoherence <- function(combined_rna_atac_graph, rna_spatial_coords_df,
                                      n_query_cells = 5,
                                      k_top_neighbors = 10, # New parameter
                                      k_next_neighbors = 10, # New parameter
  ...
  for (q_id in query_cells) {
    ...
    current_query_cells_df_base <- data.frame(
      cell = cell_ids, query_id = q_id, type_final = "Other", stringsAsFactors = FALSE
    )
...
# --- New function: validateGraphCoherenceWithRandom (added) ---
...
if (length(to_bind) > 0) combined_coherence_df <- do.call(rbind, to_bind) else combined_coherence_df <- data.frame()
