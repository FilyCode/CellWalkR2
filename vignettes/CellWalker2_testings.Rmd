...
#'   on the provided feature matrix (fast per-cell dot-products with precomputed norms).
#' - Computes Median Feature Similarity to Feature-Graph Neighbors (MFS-FGN) as a sanity check.
#' - Computes Median Feature-Graph Distance to Spatial Neighbors (MFD-SN) using direct similarity
#'   from the feature_graph (reverted to simpler, known-working logic).
...
#'     \item `plot_dist_graph_for_spatial_nn`: ggplot object for MFD-SN density (direct sim approx).
...
assessSpatialGraphCoherenceOverall <- function(
  ...
  if (num_cells <= 1) {
    ...
    return(list(
      ...
      plot_dist_graph_for_spatial_nn = empty_plots(),
  ...
  if (use_pca_embedding) {
    ...
    dense_mat <- as.matrix(feature_matrix_for_similarity)
    sv <- irlba::irlba(dense_mat, nv = n_pca_embed)
    # cell embedding: columns -> cells; compute embedding as V %*% diag(d)
    feature_matrix_embed <- t(sv$v %*% diag(sv$d))
    # The matrix `feature_matrix_embed` is k x C, where columns represent cells.
    # We need to assign cell IDs as column names so that subsetting with `cell_ids[raw_targets]` works.
    # Check dimensions before assigning names.
    if (ncol(feature_matrix_embed) == length(cell_ids)) {
        ...
        # feature_matrix_norms will be recalculated later if this block doesn't complete successfully.
        # This ensures norms are based on the matrix actually used for calculations.
        rm(dense_mat, sv); gc()
    }
  }
  ...
  # Ensure inputs are compatible for addition; Matrix handles this.
  # Average of (i,j) and (j,i) gives undirected representation.
  feature_graph_sym <- (feature_graph + Matrix::t(feature_graph)) / 2
  # Remove self-loops
  diag(feature_graph_sym) <- 0 
  
  # Debugging: Check number of edges after symmetrization
  ...
  # Remove self-loops (should already be done, but as a safeguard)
  gs <- gs[query_cell_idx != neighbor_cell_idx]
  
  if (nrow(gs) > 0) {
    message("  Calculating Metric 1 (MSD-FGN) and Metric 3 (MFS-FGN)...")
    # Calculate spatial distances for all edges
    x1 <- spatial_coords_mat[gs$query_cell_idx, 1]
    y1 <- spatial_coords_mat[gs$query_cell_idx, 2]
    x2 <- spatial_coords_mat[gs$neighbor_cell_idx, 1]
    y2 <- spatial_coords_mat[gs$neighbor_cell_idx, 2]
    gs[, spatial_distance := sqrt((x1 - x2)^2 + (y1 - y2)^2)]
    
    # For each query cell, get its top-k neighbors by similarity (x)
    # Ensure k_neighbors is not larger than available neighbors for a cell
    graph_neighbors_ranked <- gs[, head(.SD[order(-x)], k_neighbors), by = query_cell_idx]
    
    if (nrow(graph_neighbors_ranked) > 0) {
        # Calculate median spatial distance to these graph neighbors (Metric 1)
        median_spatial_results <- graph_neighbors_ranked[, .(median_dist = median(spatial_distance, na.rm = TRUE)), by = query_cell_idx]
        # Calculate median graph similarity to these graph neighbors (Metric 3)
        median_graph_sim_results_of_graph_nn <- graph_neighbors_ranked[, .(median_graph_sim = median(x, na.rm = TRUE)), by = query_cell_idx]
        
        # Map results back to the full vector
  ...
  median_graph_dist_of_spatial_neighbors_vec <- rep(NA_real_, num_cells) # Metric 4 (REVERTED LOGIC)
  
  # --- Batch processing function (per-batch) ---
  # This function calculates Metric 2 (MFS-SN) and Metric 4 (MFD-SN)
  process_batch_fast <- function(sources) {
    nb <- length(sources)
    res_metric2 <- numeric(nb) # Median Feature Similarity to Spatial Neighbors (MFS-SN)
    res_metric4 <- numeric(nb) # Median Feature-Graph Distance to Spatial Neighbors (MFD-SN)
    ...
    for (ii in seq_along(sources)) {
      ...
      mat2 <- feature_matrix_for_similarity_to_use[, cell_ids[raw_targets], drop = FALSE]
      dot_products <- as.vector(crossprod(vec1, mat2))
      norm1 <- feature_matrix_norms[i]
      norm2 <- feature_matrix_norms[raw_targets]
      denom <- norm1 * norm2
      cos_sim <- ifelse(denom == 0, 0, dot_products / denom)
      res_metric2[ii] <- median(cos_sim, na.rm = TRUE)
      
      # --- Metric 4: Median Feature-Graph Distance to Spatial Neighbors (MFD-SN) ---
      # Using the symmetrized graph for consistency
      graph_sim_to_spatial_nn <- feature_graph_sym[i, raw_targets, drop = TRUE]
      
      # Ensure numeric and handle potential NAs from sparse matrix access (if cell i is not connected to spatial neighbors)
      graph_sim_to_spatial_nn <- as.numeric(graph_sim_to_spatial_nn)
      graph_sim_to_spatial_nn[is.na(graph_sim_to_spatial_nn)] <- 0 # Treat no connection as similarity 0
      
      # Convert similarity to distance: distance = 1 - similarity
      graph_dist_to_spatial_nn <- 1 - graph_sim_to_spatial_nn
      ...
      res_metric4[ii] <- median(graph_dist_to_spatial_nn, na.rm = TRUE)
    }
    list(idx = sources, metric2_med = res_metric2, metric4_med = res_metric4)
  }
  ...
  message("  Calculating Metric 2 (MFS-SN) and Metric 4 (MFD-SN) in parallel batches...")
  ...
  for (br in batch_results) {
    ...
    median_graph_dist_of_spatial_neighbors_vec[br$idx] <- br$metric4_med
  }
  ...
  coherence_df <- data.frame(
    ...
    median_spatial_dist_of_graph_neighbors = median_spatial_dist_of_graph_neighbors_vec, # Metric 1
    median_graph_sim_of_spatial_neighbors = median_graph_sim_of_spatial_neighbors_vec,     # Metric 2
    median_graph_sim_of_graph_neighbors = median_graph_sim_of_graph_neighbors_vec,         # Metric 3
    median_graph_dist_of_spatial_neighbors = median_graph_dist_of_spatial_neighbors_vec, # Metric 4
    stringsAsFactors = FALSE
  )
  data.table::setDT(coherence_df)
  
  # Calculate number of available metrics per cell (useful for diagnostics)
  coherence_df$num_metrics_available <- rowSums(!is.na(coherence_df[, .(
    median_spatial_dist_of_graph_neighbors,
  ...
  # This step is crucial. If all metrics are NA for all cells, the df becomes empty.
  ...
  # This score combines the two distance metrics (Metric 1 and Metric 4)
  # Higher score means better coherence (lower distance for both)
  
  # Initialize score column
  coherence_df$combined_coherence_score <- NA_real_
  
  # Calculate only if there are cells with valid data for both metrics
  valid_idx_for_combined <- which(!is.na(coherence_df$median_spatial_dist_of_graph_neighbors) & !is.na(coherence_df$median_graph_dist_of_spatial_neighbors))
  
  if (length(valid_idx_for_combined) > 0) {
      msd_vals <- coherence_df$median_spatial_dist_of_graph_neighbors[valid_idx_for_combined]
      mfd_vals <- coherence_df$median_graph_dist_of_spatial_neighbors[valid_idx_for_combined]
      
      # Normalize distances to [0,1] and invert so higher is better (lower distance -> higher score)
      # Min-max normalization
      min_msd <- min(msd_vals, na.rm = TRUE)
      max_msd <- max(msd_vals, na.rm = TRUE)
      norm_msd <- if (max_msd == min_msd) rep(0.5, length(msd_vals)) else 1 - (msd_vals - min_msd) / (max_msd - min_msd)
      
      min_mfd <- min(mfd_vals, na.rm = TRUE)
      max_mfd <- max(mfd_vals, na.rm = TRUE)
      norm_mfd <- if (max_mfd == min_mfd) rep(0.5, length(mfd_vals)) else 1 - (mfd_vals - min_mfd) / (max_mfd - min_mfd)
      
      # Average the normalized, inverted scores
      coherence_df$combined_coherence_score[valid_idx_for_combined] <- (norm_msd + norm_mfd) / 2
  }
  ...
  plot_dist_graph_for_spatial_nn <- ggplot2::ggplot(coherence_df, ggplot2::aes(x = median_graph_dist_of_spatial_neighbors)) +
    ggplot2::geom_density(fill = "lightgoldenrod", alpha = 0.7, na.rm = TRUE) +
    ggplot2::labs(title = paste0(modality_name, ": Feature-Graph Dist to Spatial NN (k=", k_neighbors, ")"), x = "Median Feature-Graph Distance", y = "Density") +
    ggplot2::theme_minimal() + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5))
  ...
  if (nrow(coherence_df) > 0) {
      ...
      plot_data_long <- plot_data_spatial %>%
        tidyr::pivot_longer(cols = c(median_spatial_dist_of_graph_neighbors,
                                     median_graph_sim_of_spatial_neighbors,
                                     median_graph_sim_of_graph_neighbors,
                                     median_graph_dist_of_spatial_neighbors,
      ...
      plot_data_long_normalized <- plot_data_long %>%
        ...
        dplyr::mutate(
          ...
          # For distance metrics, higher normalized value means closer (1-norm_val)
          # For similarity and score metrics, higher normalized value is better (norm_val)
