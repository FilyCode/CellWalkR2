...
all_tissue_results <- foreach(file_path = tissue_files, 
                              ...
                              .combine = 'list', # <<< Use 'list' to keep worker results separate
                              .init = list()) %dopar% { 
      ...
      captured_output <- NULL # Variable to store all captured messages/warnings/output
      
      # Use tryCatch for robust error handling: if an error occurs for one tissue,
      # it won't stop the entire parallel job.
      tryCatch({
        # Capture all standard output and messages for this entire tissue processing block
        captured_output <- capture.output({ 
          ...
          if ("RNA" %in% names(tissue_seurat@assays)) {
            ...
            if (!is.null(current_data_matrix) && prod(dim(current_data_matrix)) > 0) {
              ...
              return(list(omicSig = NULL, tissueName = current_tissue_name, messages = captured_output)) # Return captured output
            }
          } else {
          ...
          if (is.null(results_table_mast) || nrow(results_table_mast) == 0) {
            ...
            if (nrow(sig_genes) == 0) {
              ...
              return(list(omicSig = omic_sig_object, tissueName = current_tissue_name, messages = captured_output)) # Return captured output
            }
          } # End if/else for results_table_mast
        }) # End capture.output block
      }, error = function(e) {
        # Catches any error during tissue analysis and logs it without stopping the script.
        # Ensure error messages are also added to captured output
        error_message <- paste0("  ERROR: An unhandled error occurred for tissue '", current_tissue_name, "': ", e$message)
        captured_output <- c(captured_output, error_message) # Append error message to captured output
        message(error_message) # Print to worker's stderr/stdout before being captured
        return(list(omicSig = NULL, tissueName = current_tissue_name, messages = captured_output)) 
      }) 
    } # End foreach loop
