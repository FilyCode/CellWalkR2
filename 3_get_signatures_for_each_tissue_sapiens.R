...
for (file_path in tissue_files) {
  ...
  sce_tissue <- as.SingleCellExperiment(tissue_seurat, assay = "data")
  ...
  expressed_genes <- rowSums(assay(sce_tissue, "data") > 0) / ncol(sce_tissue) > min_expressed_gene_threshold
  ...
  tryCatch({
    ...
    # Fit the ZLM model: gene ~ age + sex + subject
    ...
    if (is.null(results_table_mast) || nrow(results_table_mast) == 0) {
      ...
      # Filter significant genes for the signature
      ...
      if (nrow(sig_genes) == 0) {
        ...
        aging_signature_collection$OmicSigList[[current_tissue_name]] <- omic_sig_tissue
